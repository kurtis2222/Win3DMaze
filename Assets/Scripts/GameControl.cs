...
public class GameControl : MonoBehaviour
{
	...
	void CreateMaze()
	{
		...
		tmp.renderer.material = AssetLoader.materials[setup.wall];
		for(int i = 0; i < MAZE_SIZE; i++)
		{
			for(int j = 0; j < MAZE_SIZE; j++)
			{
	...
	void CreateWall(GameObject obj, int input, Vector3 pos)
	{
		...
		if(Random.Range(0, 100) < 5) obj.renderer.material = AssetLoader.materials[setup.wall2];
	...
	Transform CreateSprite(Vector3 pos, string typ)
	{
		...
		tmp.renderer.material = AssetLoader.sprites[typ];
	...
	Transform CreateMapObj(Transform tran, string typ, Vector3 offs)
	{
		...
		mapobj.target = tran;
		mapobj.offs = offs;
		tmp.renderer.material = AssetLoader.map_ico[typ];
		tmp.layer = map_layer;
		return tmp.transform;
	}
	...
	Transform CreateSphere(Vector3 pos, int typ)
	{
		...
		tmp.renderer.material = AssetLoader.map_ico[setup.map_sphere];
		tmp.transform.parent = mapobjs;
		tmp.layer = map_layer;
		//
		spr.map_obj = tmp;
